...
class Seesaw:
	...
	def pinMode(self, pin,  mode):
		...
		self.pinModeBulk(1 << pin, mode)


	def digitalWrite(self, pin,  value):

		self.digitalWriteBulk(1 << pin, value)


	def digitalRead(self, pin):

		return self.digitalReadBulk((1 << pin)) != 0


	def digitalReadBulk(self, pins):
	...
	def setGPIOInterrupts(self, pins, enabled):
	...
	def pinModeBulk(self, pins, mode):
	...
	def digitalWriteBulk(self, pins, value):
	...
	def EEPROMWrite8(self, addr,  val):
		self.EEPROMWrite(addr, bytearray([val]))


	def EEPROMWrite(self, addr,  buf):

		self.write(SEESAW_EEPROM_BASE, addr, buf)


	def EEPROMRead8(self, addr):

		return self.read8(SEESAW_EEPROM_BASE, addr)


	def UARTSetBaud(self, baud):
